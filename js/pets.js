  ...
  container.innerHTML = `
    <div class="card fade-in">
      <div class="card-header" style="flex-wrap: wrap; gap: 16px;">
        <h3 class="card-title"><i data-lucide="users"></i> Cadastro de Pacientes</h3>
        <div style="display: flex; gap: 12px; margin-left: auto;">
          <button id="btn-add-pet" class="btn btn-primary"><i data-lucide="user-plus"></i> Novo Paciente</button>
        </div>
      </div>
      ...
      <div style="margin: 24px 0 16px; position: relative;">
        <div class="input-wrapper" style="max-width: 400px;">
          ...
          <input type="text" id="pet-search" placeholder="Buscar paciente por nome, CPF ou e-mail..." style="padding-left: 42px;">
        </div>
      </div>
...
function handleSearch(e) {
  ...
  // Filtrar grid de Pacientes
  const filteredPets = petsList.filter(pet => {
    return (pet.name || "").toLowerCase().includes(query) || 
           (pet.cpf || "").includes(query) || 
           (pet.email || "").toLowerCase().includes(query);
  });
...
function renderPetsGrid(items = petsList) {
  ...
  if (items.length === 0) {
    container.innerHTML = `
      <div style="text-align: center; padding: 40px; color: var(--color-gray-400);">
        <i data-lucide="user" style="width: 48px; height: 48px; margin-bottom: 12px;"></i>
  ...
  items.forEach(pet => {
    const photoUrl = pet.photoUrl || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200";
    ...
    html += `
      <div class="card" style="margin-bottom: 0; padding: 20px; display: flex; flex-direction: column; gap: 12px; transition: transform var(--transition-fast);" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
        ...
        <div style="border-top: 1px solid var(--color-gray-100); padding-top: 12px; font-size: 13px; color: var(--color-gray-600); display: flex; flex-direction: column; gap: 4px;">
          <span><strong>CPF:</strong> ${pet.cpf || 'Não Informado'}</span>
        ...
        <button class="btn btn-primary btn-block btn-view-pet" data-id="${pet.id}" style="margin-top: auto; padding: 8px;">
          <i data-lucide="file-text" style="width: 16px; height: 16px;"></i> Prontuário & Detalhes
        </button>
      </div>
    `;
  });
    ...
    <form id="form-modal-pet">
      <div class="form-group">
        ...
        <div class="input-wrapper">
          ...
          <input type="text" id="p-name" required placeholder="Nome do Paciente">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          <label for="p-phone">Telefone / WhatsApp</label>
          <div class="input-wrapper">
      ...
      <div class="form-group">
        <label for="p-allergies">Alergias / Observações Médicas</label>
        <div class="input-wrapper">
          <i data-lucide="alert-circle" class="input-icon"></i>
          <input type="text" id="p-allergies" placeholder="Ex: Alergia a Dipirona, Hipertensão...">
        </div>
      </div>
      <div class="form-group">
      ...
      <button type="submit" class="btn btn-primary btn-block">Salvar Cadastro do Paciente</button>
    </form>
  `);
...
export async function showPetDetails(petId) {
  try {
    ...
    // Migração automática de exames legados
    ...
    if (examsUpdated) {
      ...
      showToast("Exames antigos migrados para a galeria do Paciente!", "info");
    }
      ...
      <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 24px; max-height: 75vh; text-align: left;">
        ...
        <div style="border-right: 1px solid var(--color-gray-200); padding-right: 20px; display: flex; flex-direction: column; gap: 20px;">
          <div style="text-align: center;">
            ...
            <span class="status-badge status-badge-success" style="margin-top: 4px;">${pet.gender || 'Não Informado'}</span>
          ...
          <div style="background: var(--color-gray-50); padding: 14px; border-radius: var(--radius-md); font-size: 13px; line-height: 1.5;">
            ...
            <p><strong>CPF:</strong> ${pet.cpf || 'Não Informado'}</p>
            <p><strong>Telefone:</strong> ${pet.phone || 'Não Informado'}</p>
            <p><strong>E-mail:</strong> ${pet.email || 'Não Informado'}</p>
            <p><strong>Alergias:</strong> ${pet.allergies || 'Nenhuma registrada'}</p>
          </div>
          ...
          <div style="text-align: center; background: var(--color-gray-50); padding: 14px; border-radius: var(--radius-md);">
            <h4 style="font-size: 12px; font-weight: 700; margin-bottom: 8px;">Carteira de Saúde Digital</h4>
            <img src="${qrCodeUrl}" alt="QR Code" style="width: 120px; height: 120px; margin: 0 auto 8px; border: 1px solid var(--color-gray-200);">
    ...
    showModal(`Ficha do Paciente: ${pet.name}`, detailsHTML);
    ...
    console.error(err);
    showToast("Erro ao abrir ficha do paciente.", "error");
  }
}
      ...
      try {
        ...
        if (salePrice > 0) {
          await addTenantDoc("finance", {
            type: "income",
            description: `Aplicação de vacina: ${vacName} no paciente ${pet.name}`,
            amount: salePrice,
...
function openEditPetModal(petId, pet) {
  showModal("Editar Paciente", `
    <form id="form-modal-edit-pet">
      <div class="form-group">
        <label for="ep-name">Nome Completo</label>
        <div class="input-wrapper">
          <i data-lucide="user" class="input-icon"></i>
          <input type="text" id="ep-name" required value="${escapeHTML(pet.name)}" placeholder="Nome do Paciente">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          ...
          <div class="input-wrapper">
            <i data-lucide="file-text" class="input-icon"></i>
            <input type="text" id="ep-cpf" value="${escapeHTML(pet.cpf || '')}" placeholder="000.000.000-00">
          </div>
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="ep-gender">Gênero</label>
          <div class="input-wrapper">
            <i data-lucide="users" class="input-icon"></i>
            <select id="ep-gender" required>
              <option value="Masculino" ${pet.gender === 'Masculino' ? 'selected' : ''}>Masculino</option>
              <option value="Feminino" ${pet.gender === 'Feminino' ? 'selected' : ''}>Feminino</option>
              <option value="Outro" ${pet.gender === 'Outro' ? 'selected' : ''}>Outro</option>
            </select>
          </div>
        </div>
        <div class="form-group">
          <label for="ep-phone">Telefone / WhatsApp</label>
          <div class="input-wrapper">
            <i data-lucide="phone" class="input-icon"></i>
            <input type="tel" id="ep-phone" required value="${escapeHTML(pet.phone || '')}" placeholder="(11) 99999-9999">
          </div>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="input-wrapper">
          <i data-lucide="mail" class="input-icon"></i>
          <input type="email" id="ep-email" value="${escapeHTML(pet.email || '')}" placeholder="email@exemplo.com">
        </div>
      </div>
      <div class="form-group">
        <label for="ep-allergies">Alergias / Observações Médicas</label>
        <div class="input-wrapper">
          <i data-lucide="alert-circle" class="input-icon"></i>
          <input type="text" id="ep-allergies" value="${escapeHTML(pet.allergies || '')}" placeholder="Alergias, doenças crônicas...">
        </div>
      </div>
      <div class="form-group">
        <label>Foto do Paciente</label>
  ...
  document.getElementById("form-modal-edit-pet").addEventListener("submit", async (e) => {
    ...
    const data = {
      ...
      cpf: document.getElementById("ep-cpf").value,
    ...
    try {
      ...
      showToast("Cadastro do paciente atualizado!", "success");
      hideModal();
      await refreshData();
      renderPetsGrid();
    } catch (err) {
...
async function handleDeletePet(petId, petName) {
  if (confirm(`ATENÇÃO! Tem certeza que deseja excluir o paciente ${petName}? Todos os prontuários e histórico de vacinas dele serão perdidos definitivamente.`)) {
    try {
